  ...
  context "without git user's email set" do
    ...
    should 'raise NoGitUserEmail' do
      assert_raise Bueller::NoGitUserEmail do
        Bueller::Generator.new(:project_name => @project_name, :user_name => @git_name, :testing_framework => :shoulda, :documentation_framework => :rdoc)
      end
    end
  end
  ...
  context "without github username set" do
    ...
    should 'raise NotGitHubUser' do
      assert_raise Bueller::NoGitHubUser do
        Bueller::Generator.new(:project_name => @project_name, :user_name => @git_name, :user_email => @git_email, :testing_framework => :shoulda, :documentation_framework => :rdoc)
      end
    end
  end
  ...
  context "without github token set" do
    ...
    should 'raise NoGitHubToken if creating repo' do
      assert_raise Bueller::NoGitHubToken do
        Bueller::Generator.new(:project_name => @project_name, :user_name => @git_name, :user_email => @git_email, :github_username => @github_user, :create_repo => true, :testing_framework => :shoulda, :documentation_framework => :rdoc)
      end
    end
  end
