...
class TestApplication < Test::Unit::TestCase
  ...
  def stub_options(options)
    ...
    stub(Bueller::Generator::Options).new { options }
  ...
  context "when options indicate help usage" do
    setup do
      ...
      stub(Bueller::Generator).new { raise "Shouldn't have made this far"}
  ...
  context "when options are good" do
    setup do
      ...
      @generator = "generator"
      stub(@generator).run
      stub(Bueller::Generator).new { @generator }
    ...
    should "create generator with options" do
      assert_received(Bueller::Generator) {|subject| subject.new(@options) }
    end
